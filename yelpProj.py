#Tiffany Nguyen, Shelley Han, Catherine Fudal
#Section 001
...
def query():
    ...
    #Prevent the program to crash if Yelp doesn't have data of the options the user is looking foor
    try:
        params = {}
...
def main():
    ...
    #Create combo box to show a list of restaurants
    drop = []
    for i in range(len(rest)):
...
def graph():
    ...
    for i in range(len(rest)):
        ...
        #What to do when there is no price information
        try:
            b['price']
        except:
            p.append(0)
    ...
    #Create graph and show it on GUI
...
def query3():
    ...
    #Create pins; pin's color changes based on ratio
    for i in range(len(rest)):
        if rat[i] > 2:
    ...
    #Add pin of user's location when provided
    c = userInput3.get()
    if c != '':
...
def query1(dlong, dlat, yourLoc='-75.339692,40.037123'):
    ...
    try:
        ...
        #Prevent program from crashing if coordinates are wrong or can't find directions
...
#Function displays directions on map
def dmap():
    global userInput2
    global window
    global long
    ...
    #Create pins for starting/ending location and create a line for the path
    pin1= "pin-s-a+000"+f'({loc(userInput2.get())})' #black start
    pin2= "pin-s-b+ff0000"+f'({long},{lat})' #red end
    path= "path-5+829dfc-0.75"+f'({pathline})' #blue path
    size2 = "500x300"
