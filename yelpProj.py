...
#Function runs query to get statis map
def query3():
    a = ""
    lst = []
    ...
    if c != '':
        a = a + lst[19] + f",pin-s-y+66c2ff({loc(c)})" 
    else:
        a = a + lst[19]
...
def detail():
    ...
    try:
        ...
        if a['hours'][0]['is_open_now'] == "True":
            label5 = tkinter.Label(win, text = f"Open now: Yes")
        else:
            label5 = tkinter.Label(win, text = f"Open now: No")
    ...
    button3 = tkinter.Button(win, text="Show Directions", command=lambda:[direction(query1(long,lat,loc(userInput2.get()))), dmap()])
