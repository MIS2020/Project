...
def ratio():
    ...
    bar1 = FigureCanvasTkAgg(figure1, win)
    bar1.get_tk_widget().grid(row = 0)
    df1.plot(kind='bar', x= 'Restaurant', y = 'Ratio', color = "#000099", ax=ax1)
...
def mapp(url):
    ...
    label = tkinter.Label(root, image=tk_image, bg='#829dfc')
...
def dmap():
    ...
    pil_image = Image.open(data_stream)
    tk_image = ImageTk.PhotoImage(pil_image)
    label = tkinter.Label(window, image=tk_image, bg='#829dfc')
    label.grid()
    win.mainloop() #Why must have this for map to appear?
